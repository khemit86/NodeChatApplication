...
$conn 		= 		new mysqli($dbrow[0],$dbrow[1],$dbrow[2],$dbrow[3]);//initiate the database connection 
...
$resultChat = $conn_driver->query($GetChatMessageSql);
$totalRecordChat = $resultChat->num_rows;

#########End######################

if (!empty($_REQUEST['driverId']) && !empty($_REQUEST['staffId']) && $_REQUEST['senderType'] == 'driver' ) {
    $driverId = $_REQUEST['driverId'];
    $userId = $driverId;
    $type = 'driver';
	
	$receiverId = $_REQUEST['staffId'];
    ...
    if (!empty($rowDriverData['DrvProfilePic']) && file_exists('../../driver-profile-picture/original-pic/' . $rowDriverData['DrvProfilePic'])) {
        $image = SITE_URL . "secure/planning/" . DRIVER_THUMB_IMAGE_DIR_50x50 . $rowDriverData['DrvProfilePic'];
    } else {
        $image = "";
    }
} else if (!empty($_REQUEST['staffId']) && !empty($_REQUEST['driverId']) && $_REQUEST['senderType'] == 'staff' ) {
   ...
   $staffId = $_REQUEST['staffId'];
    ...
    $resultStaff = $conn->query($GetStaffDetailSql);
    $totalStaffRecord = $resultStaff->num_rows;
...
if (!empty($totalRecordChat)) {
    while ($row = $resultChat->fetch_assoc()){
        ...
        if ($row['senderType'] == "staff") {
            ...
            $resultsStaff = $conn->query($StaffSql);

            $rowStaff = $resultsStaff->fetch_assoc();
            $nameStaff = explode(" ", $rowStaff['CustName']);
            //pr($rowStaffData);
            $name = $nameStaff[0];
            $img = SITE_URL."secure/planning/driver-profile-picture/no_image.jpg";
            ...
            $resultsDriver = $conn_driver->query($DriverSql);

            $rowDriver = $resultsDriver->fetch_assoc();
            ...
            if (!empty($rowDriver['DrvProfilePic']) && file_exists('../../driver-profile-picture/original-pic/' . $rowDriver['DrvProfilePic'])) {
                $img = SITE_URL . "secure/planning/" . DRIVER_THUMB_IMAGE_DIR_50x50 . $rowDriver['DrvProfilePic'];
            } else {
                $img = SITE_URL."secure/planning/driver-profile-picture/no_image.jpg";
            }
        }
        ?>
                            <li class="<?php echo $class ?>"><div class="image"><img src="<?php echo $img; ?>"><b><?php echo $name; ?></b><i data-time="<?php echo strtotime($row['created']); ?>" class="timesent"><?php echo getRelativeTime($row['created']); ?></i> </div><p><?php echo $row['message']; ?></p></li>
                            <!--
        ...
        <script type="text/javascript">
            ...
            $(function() {
				...
				socket.on('send_chat_notification_request', function(data) {
					chat_request = data;
					...
					type:"POST",
					async:false,
					data:{ReceiverId:data.ReceiverId,SenderId:data.SenderId,ThreadId:data.ThreadId},
                ...
                chatForm.on('submit', function(e) {
                    ...
                    if (textarea.val().trim().length) {
						...
						/* <?php if (!empty($_REQUEST['driverId']) && $totalRecordChat == 0 && !empty($_REQUEST['staffId'])) { ?>
                            socket.emit("save_msg_agent_initiate", {threadId: '<?php echo $threadId; ?>', staffId: '<?php echo $userId; ?>',
                                driverId: '<?php echo $driverId; ?>',companyId:'<?php echo $CompanyId ?>', message: textarea.val()});
							<?php }
						?> */
                      ...
                      // Send the message to the other person in the chat
                       socket.emit('save_msg', {msg: textarea.val(), SenderId: '<?php echo $userId; ?>', user: '<?php echo $userName; ?>', img: img, type: '<?php echo $type; ?>', threadId: '<?php echo $threadId; ?>',CompanyId:'<?php echo $CompanyId ?>',receiverId:'<?php echo $receiverId;?>',SiteUrl:'<?php echo SITE_URL; ?>'});
